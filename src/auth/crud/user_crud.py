...
from auth.models import RoleModel, UserModel
...
def get_user(session: Session, user_id: int = None, email: str = None):
...
def create_user(session: Session, user: UserInSchema):
    if get_user(session=session, email=user.email) is not None:
...
def get_or_create_user(session: Session, user: UserInSchema) -> UserModel:
    user_db = get_user(session=session, email=user.email)
